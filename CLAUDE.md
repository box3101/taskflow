...
- [ ] ispark-ui 컴포넌트를 우선 사용했나? (UiBadge, UiButton, UiToggle, UiDrawer, UiIcon 등 — 직접 HTML/CSS로 만들기 전에 라이브러리 먼저 확인)
...
- 경제지표/실적 발표 후 → 반드시 `market-events` API로 실제 결과(actual) + 메모 + 예상대비(impact) 등록
- 시황 일지 작성 시 → 해당 날짜의 모든 발표된 이벤트 결과도 함께 등록
- 빠뜨린 이벤트 없는지 캘린더 해당 날짜 이벤트 목록과 대조 확인
